...
def main() -> None:
    ...
    # Behavior -> ATT&CK technique id (author-time ground truth). Set by the
    # atomic converter; absent on the hand-authored scenarios (None is fine).
    custom_behavior_techniques: dict[str, str] = scenario.get(
    ...
    for behavior in behaviors_planned:
        ...
        technique_id = custom_behavior_techniques.get(behavior)
        for step_id, command in steps_list:
            ...
            # ATTiRe ground-truth label: tag each receipt with the ATT&CK
            # technique the behavior emulates (None for pre-atomic scenarios).
    ...
    log: dict[str, Any] = {
        ...
        # Per-behavior ground truth for downstream chain-coverage scoring
        # (detection/coverage.py). Additive; older logs simply lack these.
        "custom_behavior_produces": custom_produces,
        "custom_behavior_techniques": custom_behavior_techniques,
    }
